...
(() => {
  ...
  // ── Config ─────────────────────────────────────────────────────────────────
  // Feed is pre-generated by .github/workflows/sync-blog.yml and served as a
  // static file from the same origin — no CORS, no third-party proxy needed.
  const BLOG_JSON = 'static/data/blog.json';
  const CACHE_KEY = 'blog_feed_v2';
  ...
  // Loads from static/data/blog.json (same-origin, generated by GitHub Actions).
  // Medium blocks all known third-party RSS proxies (rss2json, allorigins, etc.)
  // so fetching live from the browser is not feasible.
  ...
  async function fetchFeed() {
    ...
    if (json.status !== 'ok') throw new Error('Invalid feed data');
